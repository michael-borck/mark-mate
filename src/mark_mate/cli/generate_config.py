...
def main(args) -> int:
    ...
    try:
        ...
        logger.info(f"  - Max cost per student: ${config['execution']['max_cost_per_student']}")
        ...
        if missing_providers:
            ...
            for provider in missing_providers:
                if provider == "anthropic":
                    ...
                    logger.warning("  - Set GEMINI_API_KEY or GOOGLE_API_KEY for Gemini grading")
        else:
            logger.info("✓ All required API keys are available")
        
        logger.info(f"\nTo use this configuration:")
        logger.info(f"  mark-mate grade extracted.json assignment.txt --config {output_file}")
