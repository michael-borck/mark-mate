        ...
        try:
            ...
            for node in ast.walk(tree):
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    ast_result["complexity_metrics"]["cyclomatic_complexity"] += (
            ...
            if ast_result["complexity_metrics"]["total_functions"] > 0:
                documented_functions = sum(
                    1 for f in ast_result["functions"] if f["has_docstring"]
                )
                ast_result["code_quality"]["function_docstring_coverage"] = (
                    documented_functions
                    / ast_result["complexity_metrics"]["total_functions"]
                )

            if ast_result["complexity_metrics"]["total_classes"] > 0:
                documented_classes = sum(
                    1 for c in ast_result["classes"] if c["has_docstring"]
                )
                ast_result["code_quality"]["class_docstring_coverage"] = (
                    documented_classes
                    / ast_result["complexity_metrics"]["total_classes"]
                )
        ...
        if "ast_analysis" in analysis_results:
            ...
            if not quality.get("has_main_guard", False):
                summary["recommendations"].append(
                    'Add main guard (__name__ == "__main__")'
                )
