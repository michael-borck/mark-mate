...
class LLMProvider:
    ...
    def get_model_name(self, provider: str, model_short_name: str) -> str:
        ...
        if model_short_name not in self.PROVIDER_CONFIGS[provider]:
            raise ValueError(f"Unsupported model {model_short_name} for provider {provider}")
    ...
    def grade_submission(self, 
        ...
        try:
            ...
            usage_info = {
                'input_tokens': response.usage.prompt_tokens if hasattr(response.usage, 'prompt_tokens') else response.usage.input_tokens,
                'output_tokens': response.usage.completion_tokens if hasattr(response.usage, 'completion_tokens') else response.usage.output_tokens,
    ...
    def get_total_usage(self) -> Dict[str, Any]:
        ...
        total_output = sum(usage['output_tokens'] for usage in self.token_usage.values())
    ...
    def reset_usage_tracking(self):
        ...
        for provider in self.token_usage:
            self.token_usage[provider] = {'input_tokens': 0, 'output_tokens': 0, 'cost': 0.0}